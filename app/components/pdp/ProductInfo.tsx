...
export default function ProductInfo({ product }: ProductInfoProps) {
  const [selectedColor, setSelectedColor] = useState(product.colors.find((c: any) => c.selected)?.name || product.colors[0]?.name);
  const [selectedVariant, setSelectedVariant] = useState(product.variants[0]?.id);

  // Calculate total price
  const variantPrice = product.variants.find((v: any) => v.id === selectedVariant)?.price || 0;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-b border-gray-100 pb-6 mb-6">
        <div className="flex justify-between items-start">
           <div>
             <h2 className="text-xs font-serif text-gray-500 uppercase tracking-widest mb-2">Neha Rajput</h2>
      ...
      <div className="mt-6 space-y-1 text-xs text-gray-500 tracking-wide">
         <p>SKU: {product.sku}</p>
         <p className={product.stockStatus === 'In Stock' ? 'text-green-600' : 'text-red-500'}>
